...
def _build_legal_time_windows(display_at: str, video_duration: float, on_screen_duration: float) -> List[Dict[str, float]]:
    safe_duration = max(float(video_duration or 0.0), 0.0)
    show_for = min(max(float(on_screen_duration or 3.0), 0.05), safe_duration if safe_duration > 0 else 3.0)
    end_start = max(0.0, safe_duration - show_for)

    if display_at == "start":
        return [{"start": 0.0, "end": show_for}]
    if display_at == "both":
        return [
            {"start": 0.0, "end": show_for},
            {"start": end_start, "end": safe_duration},
        ]
    if display_at == "entire":
        return [{"start": 0.0, "end": safe_duration}]
    return [{"start": end_start, "end": safe_duration}]


def _build_legal_ae_cues(
    legal_items: List[Any],
    ...
    for item in legal_items or []:
        raw_text = str(getattr(item, "text", "") or "")
        clean_text = re.sub(r"\*.*logo\*", "", raw_text, flags=re.IGNORECASE).strip()
        secondary_text = str(getattr(item, "text_secondary", "") or "").strip()
        display_at = str(getattr(item, "display_at", "end") or "end")
        on_screen_duration = float(getattr(item, "duration_on_screen", 3.0) or 3.0)
        position = manual_position or str(getattr(item, "position", "bottom") or "bottom")
        font_size = int(getattr(item, "font_size", 24) or 24)

        windows = _build_legal_time_windows(display_at, video_duration, on_screen_duration)
        for w in windows:
            cues.append(
                {
                    "index": cue_index,
                    ...
                    "duration": round(max(0.0, float(w["end"]) - float(w["start"])), 3),
                    "font_size": font_size,
                }
            )
            cue_index += 1
...
def _write_legal_timeline_json_for_ae(
    output_path: Path,
    cues: List[Dict[str, Any]],
    video_path: str,
    metadata: Dict[str, Any],
...
def _write_legal_ae_jsx_script(
    script_dir: Path,
    job_id: str,
    ...
    width = int(video_meta.get("width") or 1920)
    height = int(video_meta.get("height") or 1080)
    fps = float(video_meta.get("fps") or 25.0)
    duration = float(video_meta.get("duration") or 10.0)
    ...
    jsx_content = f"""/* Auto-generated by Video Subtitles Automation */
var CONFIG = {config_json};

(function () {{
    app.beginUndoGroup("Import Legal AE Package");
    ...
    var projectRoot = app.project.items.addFolder("LEGAL_AE_PACKAGE_" + CONFIG.job_id);
    var footageFolder = app.project.items.addFolder("01_FOOTAGE");
    var compFolder = app.project.items.addFolder("02_COMPS");
    footageFolder.parentFolder = projectRoot;
    compFolder.parentFolder = projectRoot;

    function joinPath(basePath, relPath) {{
        var base = String(basePath || "").replace(/\\\\/g, "/").replace(/\/+$/, "");
        var rel = String(relPath || "").replace(/\\\\/g, "/").replace(/^\/+/, "");
        return base + "/" + rel;
    }}

    function importRelativeFile(relPath) {{
        if (!relPath) return null;
        var file = new File(joinPath(packageRoot.fsName, relPath));
    ...
    var comp = app.project.items.addComp(
        "LEGAL_MAIN_" + CONFIG.width + "x" + CONFIG.height + "_" + CONFIG.fps + "fps",
        CONFIG.width, CONFIG.height, 1, CONFIG.duration, CONFIG.fps
    );
    comp.parentFolder = compFolder;

    var videoItem = importRelativeFile(CONFIG.video_rel_path);
    if (videoItem) {{
        videoItem.parentFolder = footageFolder;
        var videoLayer = comp.layers.add(videoItem);
        videoLayer.moveToEnd();
    }}

    var audioItem = importRelativeFile(CONFIG.audio_rel_path);
    if (audioItem) {{
        audioItem.parentFolder = footageFolder;
        var audioLayer = comp.layers.add(audioItem);
        audioLayer.moveToEnd();
    }}

    function positionForCue(posName) {{
        var margin = Math.round(CONFIG.height * 0.04);
        var map = {{
            "top_left": [margin, margin],
            "top": [CONFIG.width / 2, margin],
            "top_right": [CONFIG.width - margin, margin],
            "center": [CONFIG.width / 2, CONFIG.height / 2],
            "bottom_left": [margin, CONFIG.height - margin],
            "bottom": [CONFIG.width / 2, CONFIG.height - margin],
            "bottom_right": [CONFIG.width - margin, CONFIG.height - margin]
        }};
        return map[posName] || map["bottom"];
    }}

    for (var i = 0; i < CONFIG.cues.length; i++) {{
        var cue = CONFIG.cues[i];
        var txt = cue.text || "";
        if (!txt) continue;

        var start = clamp(Number(cue.start || 0), 0, CONFIG.duration);
        var end = clamp(Number(cue.end || (start + 0.1)), start + 0.05, CONFIG.duration);
        ...
        textDoc.fontSize = Math.max(24, Number(cue.font_size || Math.round(CONFIG.height * 0.03)));
        textDoc.justification = ParagraphJustification.CENTER_JUSTIFY;
        textProp.setValue(textDoc);

        textLayer.property("Position").setValue(positionForCue(String(cue.position || "bottom")));
        textLayer.name = "LEGAL_" + ("0000" + (i + 1)).slice(-4);
    }}

    var saveFile = new File(joinPath(scriptDir.fsName, "legal_project_" + CONFIG.job_id + ".aep"));
    app.project.save(saveFile);
    app.endUndoGroup();
}})();
"""

    jsx_path = script_dir / f"import_legal_ae_{job_id}.jsx"
...
def _write_ae_jsx_script(
    ...
    """
    Generate ExtendScript (.jsx) to auto-import assets and create comps in After Effects.
    """
    width = int(video_meta.get("width") or 1920)
    height = int(video_meta.get("height") or 1080)
    fps = float(video_meta.get("fps") or 25.0)
...
(function () {{
    app.beginUndoGroup("Import AE Package");

    if (!app.project) {{
        app.newProject();
    }}

    // Resolve package root dynamically from the current script location
    // so the package works on any machine/path after unzip.
    var scriptFile = new File($.fileName);
    var scriptDir = scriptFile.parent;
    var packageRoot = scriptDir.parent;
    var projectRoot = app.project.items.addFolder("AE_PACKAGE_" + CONFIG.job_id);
    var footageFolder = app.project.items.addFolder("01_FOOTAGE");
    var compsFolder = app.project.items.addFolder("02_COMPS");
    footageFolder.parentFolder = projectRoot;
    compsFolder.parentFolder = projectRoot;
    ...
    for (var i = 0; i < CONFIG.languages.length; i++) {{
        var langCfg = CONFIG.languages[i];
        var langCode = (langCfg.language || "xx").toUpperCase();
        var compName = "MAIN_" + langCode + "_" + CONFIG.width + "x" + CONFIG.height + "_" + CONFIG.fps + "fps";
        var comp = app.project.items.addComp(compName, CONFIG.width, CONFIG.height, 1, CONFIG.duration, CONFIG.fps);
        comp.parentFolder = compsFolder;

        var videoItem = importRelativeFile(langCfg.video_rel_path);
        if (videoItem) {{
            videoItem.parentFolder = footageFolder;
            var videoLayer = comp.layers.add(videoItem);
            videoLayer.moveToEnd();
        ...
        var segments = langCfg.segments || [];
        for (var j = 0; j < segments.length; j++) {{
            var seg = segments[j];
            var textValue = seg.text || "";
            var start = clamp(Number(seg.start || 0), 0, CONFIG.duration);
            var end = clamp(Number(seg.end || (start + 0.1)), start + 0.05, CONFIG.duration);
            ...
            var textProp = textLayer.property("Source Text");
            var textDoc = textProp.value;
            textDoc.fontSize = Math.max(40, Math.round(CONFIG.height * 0.045));
            textDoc.justification = ParagraphJustification.CENTER_JUSTIFY;
            textProp.setValue(textDoc);

            textLayer.property("Position").setValue([CONFIG.width / 2, CONFIG.height * 0.9]);
            textLayer.name = "SUB_" + ("0000" + (j + 1)).slice(-4) + "_" + langCode;
        }}
    }}

    var saveFile = new File(joinPath(scriptDir.fsName, "project_" + CONFIG.job_id + ".aep"));
    app.project.save(saveFile);
    app.endUndoGroup();
}})();
"""

    jsx_path = script_dir / f"import_ae_{job_id}.jsx"
    jsx_path.write_text(jsx_content, encoding="utf-8")
    return str(jsx_path)
...
async def process_subtitles(
    ...
    try:
        ...
        if ae_package_mode:
            if wav_path and ae_lang_entries:
                try:
...
async def process_legal(
    ...
    try:
        ...
        from src.legal.database import MediaType, UsageType, get_legal_database
...
async def add_legal_content(
    ...
    export_format: str = Form("legal_video"),
